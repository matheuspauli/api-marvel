...
app.get('/personagens', (req, res, next) => {
    //Limit não pode ser < 1 || > 100
    //Chave de acesso para comunicar com o servidor da Marvel
    const { limit, page } = req.query;
    const ts = new Date().getTime().toString();
    const hash = (0, md5_1.default)(ts + privateKey + publicKey);
    //---------------------------------------------------------------
    //Acesso ao servidor,
    axios_1.default.get(`${apiUrl}/characters`, {
        params: {
            ts: ts,
            apikey: publicKey,
            hash: hash,
        ...
        const info = personagens.map(personagem => {
            return {
...
app.get('/personagem/:id', (req, res, next) => {
    //Limit não pode ser < 1 || > 100
    const charId = Number(req.params.id);
    const ts = new Date().getTime().toString();
    const hash = (0, md5_1.default)(ts + privateKey + publicKey);
    // GET /v1/public/characters/{characterId}/comics
    axios_1.default.get(`${apiUrl}/characters/${charId}`, {
        params: {
            ts: ts,
            apikey: publicKey,
            hash: hash,
        ...
        let path = objRetorno.personagens[0].img.path;
        let type = objRetorno.personagens[0].img.extension;
        // -------------------------------------------------------------------------------------
        axios_1.default.get(`${apiUrl}/characters/${charId}/comics`, {
            params: {
                ts: ts,
                apikey: publicKey,
                hash: hash,
            ...
            const comicsCharacer = response.data.data.results;
            const comicsCharacerFiltred = comicsCharacer.map(comicsCharacer => {
                return {
            ...
            let comicImgPath = comicsCharacerFiltred[1].caminho.path;
            let comicImgExt = comicsCharacerFiltred[1].caminho.extension;
            let ImgComic = comicImgPath + "." + comicImgExt;
            // const objRetorno = {
            //     count: nomes.length,
            //     personagens: [...nomes]
            // };
            ...
            res.send(`<h1>Nome:${objRetorno.personagens[0].nome}</h1>
